...
module.exports = class Customer {
    ...
    updateById(id,customer, cbResult){
        let queryString = 'Update customer SET email = ?, firstname = ?, lastname = ?, username = ?, birthday = ?';
        queryString += ' WHERE id = ?';
        sql.query(queryString,
            [customer.email, customer.firstname, customer.lastname,customer.username, customer.birthday, parseInt(id)],
            (err, result) => {
                if (err){
                    console.log("error: ", err);
                    //err zurückgeben, data = null
